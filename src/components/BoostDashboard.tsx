...
export function BoostDashboard() {
  ...
  console.log('LOG: COMPONENT-BOOST-2 - BoostDashboard rendered');
  ...
  const loadReputationData = async () => {
    console.log('LOG: COMPONENT-BOOST-3 - Loading reputation data');
    setLoading(true);
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('LOG: COMPONENT-BOOST-4 - Reputation data loaded');
      }
    } catch (error) {
      console.error('LOG: COMPONENT-BOOST-ERROR-1 - Failed to load reputation data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const searchMentions = async () => {
    console.log('LOG: COMPONENT-BOOST-5 - Searching for mentions');
    setLoading(true);
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('LOG: COMPONENT-BOOST-6 - Mentions search completed');
      }
    } catch (error) {
      console.error('LOG: COMPONENT-BOOST-ERROR-2 - Mentions search failed:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createSeedingPlan = async (strategy: string) => {
    console.log('LOG: COMPONENT-BOOST-7 - Creating seeding plan:', strategy);
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('LOG: COMPONENT-BOOST-8 - Seeding plan created');
      }
    } catch (error) {
      console.error('LOG: COMPONENT-BOOST-ERROR-3 - Failed to create seeding plan:', error);
    }
  };
  ...
  const getSentimentIcon = (score: number) => {
    if (score > 0.3) return <CheckCircle className="w-4 h-4" />;
    if (score < -0.3) return <AlertTriangle className="w-4 h-4" />;
...
export function useBoostData() {
  ...
  const fetchBoostData = async (action: string, params: unknown = {}) => {
    ...
    try {
      ...
      console.error('LOG: COMPONENT-BOOST-ERROR-4 - Hook fetch failed:', error);
      return null;
    } finally {
      setLoading(false);
    }

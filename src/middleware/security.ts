...
export class SecurityMiddleware {
    ...
    this.config = {
      ...
      csp: {
        directives: {
          ...
          'script-src': [
            "'self'",
            // 'unsafe-inline' removed for security - use nonces instead
  ...
  handlePreflight(request: Request): Response {
    ...
    logger.info('CORS preflight request', {
      ...
      metadata: { _origin, method, headers }
    });
   ...
   */
  createSecurityViolationResponse(reason: string): Response {
    logger.security('Security violation detected', 'high', {
      component: 'SecurityMiddleware',
      action: 'securityViolation',
      metadata: { _reason, userAgent, ip }
    });
...
export function createSecurityMiddleware() {
  ...
  return {
    ...
    processRequest: (request: Request): Response | null => {
      ...
      if (!validation.valid) {
        return security.createSecurityViolationResponse(validation.reason || 'Unknown security violation');
      }

...
export class SecurityAuditLogger {
  ...
  async logSecurityEvent(event: Omit<SecurityEvent, 'id' | 'timestamp'>): Promise<void> {
    try {
      ...
      console.log(`LOG: SECURITY-AUDIT-1 - Event logged: ${sanitizedEvent.type} (${sanitizedEvent.severity})`);
    } catch (error: unknown) {
  ...
  async getSecurityMetrics(
    ...
    try {
      ...
      console.error('LOG: SECURITY-METRICS-ERROR-1 - Failed to get security metrics:', error);
  ...
  async searchEvents(
    ...
    try {
      ...
      console.error('LOG: SECURITY-SEARCH-ERROR-1 - Failed to search security events:', error);
      throw new ValidationError(
  ...
  private sanitizeStringForPII(str: string): string {
    ...
    sanitized = sanitized.replace(/+?[ds-()]{10,}/g, '[PHONE]');

    // Credit card patterns (simplified)
    sanitized = sanitized.replace(/bd{4}[s-]?d{4}[s-]?d{4}[s-]?d{4}b/g, '[CARD]');

    // SSN patterns
    sanitized = sanitized.replace(/bd{3}-?d{2}-?d{4}b/g, '[SSN]');
  ...
  private containsPII(str: string): boolean {
    ...
    // Phone pattern
    if (/+?[ds-()]{10,}/.test(str)) return true;

    // Credit card pattern
    if (/bd{4}[s-]?d{4}[s-]?d{4}[s-]?d{4}b/.test(str)) return true;

    // SSN pattern
    if (/bd{3}-?d{2}-?d{4}b/.test(str)) return true;
  ...
  private async flushBatch(): Promise<void> {
    ...
    try {
      ...
      console.log(`LOG: SECURITY-AUDIT-FLUSH-1 - Flushed ${events.length} security events`);
    } catch (error: unknown) {
      console.error('LOG: SECURITY-AUDIT-FLUSH-ERROR-1 - Failed to flush security events:', error);
  ...
  private startBatchFlush(): void {
    this.flushTimer = setInterval(() => {
      this.flushBatch().catch(error => {
        console.error('LOG: SECURITY-AUDIT-TIMER-ERROR-1 - Timer flush failed:', error);
      });
    }, this.flushInterval);
  }
  ...
  private async sendCriticalAlert(event: SecurityEvent): Promise<void> {
    try {
      ...
      console.error('LOG: SECURITY-ALERT-ERROR-1 - Failed to send critical alert:', error);
    }
  }
  ...
  async cleanupOldEvents(retentionDays: number = 90): Promise<{ deleted: number }> {
    try {
      ...
      console.log(`LOG: SECURITY-CLEANUP-1 - Deleted ${result.meta?.changes || 0} old security events`);

      return { deleted: result.meta?.changes || 0 };
    } catch (error: unknown) {
      console.error('LOG: SECURITY-CLEANUP-ERROR-1 - Failed to cleanup old events:', error);
      return { deleted: 0 };
    }
  }
  ...
  destroy(): void {
    ...
    this.flushBatch().catch(error => {
      console.error('LOG: SECURITY-AUDIT-DESTROY-ERROR-1 - Final flush failed:', error);
    });
  }
}
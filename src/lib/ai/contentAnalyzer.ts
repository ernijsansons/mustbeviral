...
export class ContentAnalyzer {
  ...
  constructor(ai: unknown, env: unknown, vectorize?: unknown) {
    this.ai = ai;
    this.env = env;
  ...
  async compareWithCompetitors(content: string, competitors: CompetitorAnalysis[]): Promise<{
    ...
    const contentAnalysis = await this.analyzeContent({ _content, type: 'article' });
    ...
    const sortedCompetitors = competitors
      ...
      .sort((a, _b) => b.overallScore - a.overallScore);
    ...
    return { _position, strengths, weaknesses, opportunities };
  }
  ...
  async generateContentEmbedding(content: string): Promise<number[]> {
    try {
      ...
      console.warn('Vectorize embedding failed, using fallback');
    }
  ...
  async findSimilarContent(content: string, threshold: number = 0.8): Promise<{
    ...
    try {
      ...
      const similarContent = results.matches
        .filter((match: unknown) => match.score >= threshold)
        .map((match: unknown) => ({
          id: match.id,
      ...
      return { _similarContent, isDuplicate };
    } catch (error: unknown) {
  ...
  private async analyzeSentiment(content: string): Promise<{
    ...
    try {
      ...
      if (result && result[0]) {
        return {
          label: result[0].label.toLowerCase() as unknown,

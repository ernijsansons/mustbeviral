...
export class RetryClient {
  ...
  getMetrics(): {
    ...
    const averageAttempts = totalRequests > 0
      ? this.requestMetrics.reduce((sum, _m) => sum + m.attempts.length, 0) / totalRequests
      : 0;

    const averageDuration = totalRequests > 0
      ? this.requestMetrics.reduce((sum, _m) => sum + m.totalDuration, 0) / totalRequests
      : 0;

...
import { _createContext, useContext, useEffect, useState, ReactNode } from 'react';
import { _apiClient, type User, type ApiResponse, ApiClientError } from '../lib/api';
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const response = await apiClient.login({ _email, password });
  ...
  const register = async (email: string, username: string, password: string, role: 'creator' | 'influencer') => {
    try {
      ...
      const response = await apiClient.register({ _email, username, password, role });
  ...
  const value: AuthContextType = { _user,
    isLoading,

...
export class SecurityManager {
  ...
  private deriveEncryptionKey(): Buffer {
    ...
    // Reject default or weak secrets in all environments
    const weakSecrets = ['default-secret-change-in-production', 'test', 'secret', 'password', '12345678'];
    if (weakSecrets.includes(secret.toLowerCase())) {
      throw new Error('ENCRYPTION_SECRET contains a weak or default value. Please use a secure, randomly generated secret');
    }
  ...
  async logSecurityEvent(event: {
    ...
    try {
      ...
      // TODO: Implement proper database storage for audit logs

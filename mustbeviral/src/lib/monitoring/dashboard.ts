...
class MonitoringDashboard {
  ...
  private async sendAlertNotifications(alert: AlertRule, alertData: unknown): Promise<void> {
    for (const channel of alert.notificationChannels) {
      try {
        await this.sendNotification(channel, alert, alertData);
      } catch (error) {
  ...
  private async sendNotification(channel: string, alert: AlertRule, alertData: unknown): Promise<void> {
    const message = this.formatAlertMessage(alert, alertData);

    switch (channel.toLowerCase()) {
      case 'console':
        log.info('Alert notification sent to console', { 
          component: 'MonitoringDashboard',
          action: 'sendNotification',
          metadata: { channel: 'console', message }
        });
        break;
      case 'email':
      ...
      case 'slack':
        ...
        break;
      case 'webhook':
        await this.sendWebhookNotification(alertData, alert);
        break;
      default:
  ...
  private formatAlertMessage(alert: AlertRule, alertData: unknown): string {
    const data = alertData as { alertName: string; metric: string; value: number; threshold: number; severity: string; timestamp: number };
    return `🚨 ALERT: ${data.alertName}\n` +
           `Metric: ${data.metric}\n` +
  ...
  private async sendEmailNotification(message: string, alert: AlertRule): Promise<void> {
  ...
  private async sendSlackNotification(message: string, alert: AlertRule): Promise<void> {
  ...
  private async sendWebhookNotification(alertData: unknown, alert: AlertRule): Promise<void> {

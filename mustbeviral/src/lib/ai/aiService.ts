...
export interface AICapabilities {
  readonly contentGeneration: boolean;
  readonly contentAnalysis: boolean;
  readonly sentimentAnalysis: boolean;
  readonly seoOptimization: boolean;
  readonly viralPrediction: boolean;
  readonly plagiarismDetection: boolean;
...
export class AIService {
  ...
  constructor(
    ...
    this.config = {
      ...
      rateLimitWindowMs: 1000 * 60, // 1 minute
      ...config
    };
...
Focus on: ${analysis.suggestions.improvements.join(', ')}`
  ...
  async createContentStrategy(
    ...
    return { strategy, contentCalendar, kpis };
  }

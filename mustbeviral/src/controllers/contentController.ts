...
export class ContentController {
  ...
  static async generateContent(
    ...
    try {
      ...
      logger.info('Generating content', { userId,
        topic: body.topic,
      ...
      for (const platform of body.platforms) {
        try {
          ...
          logger.error('Failed to generate content for platform', error as Error, { platform,
            userId
          });
        }
      }
      ...
      logger.audit('content_generated', { userId,
  ...
  static async getUserContent(
    ...
    try {
      ...
      logger.info('Retrieved user content', { userId,
  ...
  static async updateContent(
    ...
    try {
      ...
      if (body.title && typeof body.title !== 'string') {
        return new Response(JSON.stringify({ error: 'Title must be a string' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      if (body.status && !['draft', 'published', 'pending_review'].includes(body.status)) {
      ...
      logger.audit('content_updated', { userId,
        contentId,
        status: body.status
      });
  ...
  static async deleteContent(
    ...
    try {
      ...
      logger.audit('content_deleted', { userId,
        contentId
      });

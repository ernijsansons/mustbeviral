...
export const HeroSection = memo(() => {
  ...
  return (
    <section
      ...
      role="banner">
      {/* Background decorations with reduced motion support */}
      <div className="absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className={cn(
          "absolute -top-40 -right-40 w-80 h-80 bg-primary-200 rounded-full blur-3xl opacity-20",
          !prefersReducedMotion && "animate-float"
        )} />
        <div className={cn(
          "absolute -bottom-40 -left-40 w-80 h-80 bg-viral-200 rounded-full blur-3xl opacity-20",
          !prefersReducedMotion && "animate-float"
        )} style={{ animationDelay: prefersReducedMotion ? '0s' : '2s' }} />
        <div className={cn(
          "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gold-200 rounded-full blur-3xl opacity-10",
          !prefersReducedMotion && "animate-pulse"
        )} />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        {/* Badge with semantic HTML */}
        <div className={cn(
          "inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full border border-primary-200 mb-6",
        ...
        {/* Main headline with better semantics */}
        <h1 id="hero-heading" className={cn(
          "mb-6",
        ...
        {/* CTA Buttons with proper focus management */}
        <div className={cn(
          "flex flex-col sm:flex-row gap-4 justify-center mb-12",
          !prefersReducedMotion && "animate-slide-up"
        )} style={{ animationDelay: prefersReducedMotion ? '0s' : '0.3s' }} role="group" aria-label="Call to action buttons">
          <Link href="/onboard">
